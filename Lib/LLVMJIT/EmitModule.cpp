...
void LLVMJIT::emitModule(const IR::Module &irModule,
    ...
    if (moduleContext.tableOffsets.size()) { moduleContext.defaultTableOffset = moduleContext.tableOffsets[0]; }
    ...
    if (moduleContext.memoryOffsets.size()) { moduleContext.defaultMemoryOffset = moduleContext.memoryOffsets[0]; }
