            ...
            switch (unwindCode->opcode) {
                ...
                case UWOP_SAVE_NONVOL:
                    std::cout << "    0x%02x UWOP_SAVE_NONVOL %s 0x%x\n" << unwindCode->codeOffset
                              << getUnwindRegisterName(unwindCode->opInfo) << *(U16 *) &unwindCode[1] * 8;
                    unwindCode += 2;
                    break;
                case UWOP_SAVE_NONVOL_FAR:
                    std::cout << "    0x%02x UWOP_SAVE_NONVOL_FAR %s 0x%x\n" << unwindCode->codeOffset
                              << getUnwindRegisterName(unwindCode->opInfo) << *(U32 *) &unwindCode[1];
                    unwindCode += 3;
                    break;
                case UWOP_SAVE_XMM128:
                    std::cout << "    0x%02x UWOP_SAVE_XMM128 xmm%u 0x%x\n" << unwindCode->codeOffset
                              << unwindCode->opInfo << *(U16 *) &unwindCode[1] * 8;
                    unwindCode += 2;
                    break;
                case UWOP_SAVE_XMM128_FAR:
                    std::cout << "    0x%02x UWOP_SAVE_XMM128_FAR xmm%u 0x%x\n" << unwindCode->codeOffset
                              << unwindCode->opInfo << *(U32 *) &unwindCode[1];
                    unwindCode += 3;
                    break;
