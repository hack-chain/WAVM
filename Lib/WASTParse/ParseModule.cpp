...
static void parseExport(CursorState *cursor) {
    ...
    parseParenthesized(cursor, [&] {
        ...
        switch (cursor->nextToken->type) {
            case t_func:
                exportKind = ExternKind::function;
                break;
            case t_table:
                exportKind = ExternKind::table;
                break;
            case t_memory:
                exportKind = ExternKind::memory;
                break;
            case t_global:
                exportKind = ExternKind::global;
                break;
            case t_exception_type:
                exportKind = ExternKind::exceptionType;
                break;
            default:
...
void dumpHashMapSpaceAnalysis(const Map &map, const char *description) {
    if (map.size()) {
        ...
        std::cout << "%s used %.1fKB for %" PRIuPTR
                  << " elements (%.0f%% occupancy, %.1f bytes/element). Avg/max probe length: %f/%" PRIuPTR << "\n"
                  << description << totalMemoryBytes / 1024.0f << map.size() << occupancy * 100.0f
                  << F32(totalMemoryBytes) / map.size() << averageProbeCount << maxProbeCount;
    }
}
