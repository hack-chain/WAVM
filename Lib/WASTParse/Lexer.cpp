...
Token *WAST::lex(const char *string, Uptr stringLength, LineInfo *&outLineInfo) {
    ...
    if (stringLength > UINT32_MAX) { Errors::fatalf("cannot lex strings with more than %u characters", UINT32_MAX); }
...
TextFileLocus WAST::calcLocusFromOffset(const char *string,
    ...
    while (maxLineIndex > minLineIndex) {
        ...
        if (charOffset < lineInfo->lineStarts[medianLineIndex]) { maxLineIndex = medianLineIndex - 1; }
