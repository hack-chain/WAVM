...
static IR::ValueTuple parseConstExpressionTuple(CursorState *cursor) {
    IR::ValueTuple tuple;
    while (cursor->nextToken->type == t_leftParenthesis) { tuple.values.push_back(parseConstExpression(cursor)); };
    return tuple;
}
            ...
            switch (cursor->nextToken->type) {
                ...
                case t_assert_trap: {
                    ...
                    if (!strcmp(expectedErrorMessage.c_str(),
                                "out of bounds memory access")) { expectedType = ExpectedTrapType::outOfBoundsMemoryAccess; }
