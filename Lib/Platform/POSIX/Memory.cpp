...
U8 *Platform::allocateAlignedVirtualPages(Uptr numPages,
    ...
    if (alignmentLog2 > pageSizeLog2) {
        ...
        if (numHeadPaddingBytes > 0) { errorUnless(!munmap(unalignedBaseAddress, numHeadPaddingBytes)); }

        const Uptr numTailPaddingBytes = alignmentBytes - (alignedAddress - address);
        if (numTailPaddingBytes > 0) { errorUnless(!munmap(result + (numPages << pageSizeLog2), numTailPaddingBytes)); }
