...
U8 *Platform::allocateAlignedVirtualPages(Uptr numPages, Uptr alignmentLog2, U8 *&outUnalignedBaseAddress) {
    ...
    if (alignmentLog2 > pageSizeLog2) {
        ...
        U8 *unalignedBaseAddress = (U8 *) mmap(nullptr,
                                               numBytes + alignmentBytes, PROT_NONE,
...
bool Platform::commitVirtualPages(U8 *baseVirtualAddress, Uptr numPages, MemoryAccess access) {
    ...
    int result = mprotect(baseVirtualAddress, numPages << getPageSizeLog2(), memoryAccessAsPOSIXFlag(access));
    if (result != 0) {
        fprintf(stderr, "mprotect(0x%" PRIxPTR ", %" PRIuPTR ", %u) failed! errno=%s\n", reinterpret_cast<Uptr>(baseVirtualAddress),
                numPages << getPageSizeLog2(), memoryAccessAsPOSIXFlag(access), strerror(errno));
    }
    return result == 0;
}
...
void Platform::decommitVirtualPages(U8 *baseVirtualAddress, Uptr numPages) {
    ...
    if (mmap(baseVirtualAddress, numBytes, PROT_NONE, MAP_FIXED | MAP_PRIVATE | MAP_ANONYMOUS, -1, 0) == MAP_FAILED) {
        Errors::fatalf("mmap(0x%" PRIxPTR ", %" PRIuPTR
                       ", PROT_NONE, MAP_FIXED | MAP_PRIVATE | MAP_ANONYMOUS, -1, 0) failed! errno=%s", reinterpret_cast<Uptr>(baseVirtualAddress), numBytes, strerror(errno));
    }
}
...
void Platform::freeVirtualPages(U8 *baseVirtualAddress, Uptr numPages) {
    ...
    if (munmap(baseVirtualAddress, numPages << getPageSizeLog2())) {
        Errors::fatalf("munmap(0x%" PRIxPTR ", %u) failed! errno=%s", reinterpret_cast<Uptr>(baseVirtualAddress),
                       numPages << getPageSizeLog2(), strerror(errno));
    }
}
...
void Platform::freeAlignedVirtualPages(U8 *unalignedBaseAddress, Uptr numPages, Uptr alignmentLog2) {
    ...
    if (munmap(unalignedBaseAddress, numPages << getPageSizeLog2())) {
        Errors::fatalf("munmap(0x%" PRIxPTR ", %u) failed! errno=%s", reinterpret_cast<Uptr>(unalignedBaseAddress),
                       numPages << getPageSizeLog2(), strerror(errno));
    }
}
