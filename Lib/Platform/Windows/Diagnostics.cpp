...
static void dumpErrorCallStack(Uptr numOmittedFramesFromTop) {
    ...
    for (auto frame : callStack.stackFrames) {
        ...
        if (!Platform::describeInstructionPointer(frame.ip,
                                                  frameDescription)) { frameDescription = "<unknown function>"; }
...
struct DbgHelp {
    typedef BOOL(WINAPI
    * SymFromAddr)(HANDLE, U64, U64*, SYMBOL_INFO*);
...
private:
    DbgHelp() {
        ...
        if (dbgHelpModule) {
            ...
            typedef BOOL(WINAPI
            *SymInitialize)(HANDLE, PCTSTR, BOOL);
            SymInitialize
            symInitialize
                    = (SymInitialize)
...
static std::string trimModuleName(std::string moduleName) {
    ...
    if (lastBackslashOffset != UINTPTR_MAX && moduleName.size() >= lastBackslashOffset
        && moduleName.substr(0, lastBackslashOffset)
           == thisModuleName.substr(0, lastBackslashOffset)) { return moduleName.substr(lastBackslashOffset + 1); }
    else {
        return moduleName;
    }
}
