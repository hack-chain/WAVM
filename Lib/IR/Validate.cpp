...
static void validate(const IR::FeatureSpec &featureSpec, IR::ValueType valueType) {
    ...
    switch (valueType) {
        ...
        case ValueType::f32:
        case ValueType::f64:
            isValid = featureSpec.mvp;
            break;
        case ValueType::v128:
            isValid = featureSpec.simd;
            break;
        case ValueType::anyref:
        case ValueType::anyfunc:
            isValid = featureSpec.referenceTypes;
            break;
        default:
            isValid = false;
    };
...
static FunctionType validateBlockType(const Module &module, const IndexedBlockType &type) {
    switch (type.format) {
        case IndexedBlockType::noParametersOrResult:
            return FunctionType();
            ...
            for (Uptr stackIndex = 0; stackIndex < controlStack.size(); ++stackIndex) {
                ...
                switch (controlStack[stackIndex].type) {
                    ...
                    case ControlContext::Type::loop:
                        ...
                        break;
                    case ControlContext::Type::try_:
                        controlStackString += "R";
                        break;
                    case ControlContext::Type::catch_:
                        controlStackString += "C";
                        break;
                    default:
                        Errors::unreachable();
                };
