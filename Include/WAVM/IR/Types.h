...
namespace WAVM {
    namespace IR {
        ...
        enum class ValueType : U8 {
            none = 0, any = 1, i32 = 2, i64 = 3, f32 = 4, f64 = 5, v128 = 6, anyref = 7, anyfunc = 8, nullref = 9,
        ...
        static_assert(Uptr(ValueType::anyref) == Uptr(ReferenceType::anyref), "ReferenceType and ValueType must match");
        static_assert(
                Uptr(ValueType::anyfunc) == Uptr(ReferenceType::anyfunc), "ReferenceType and ValueType must match");

        inline ValueType asValueType(ReferenceType type) {
            return ValueType(type);
        }

        inline bool isReferenceType(ValueType type) {
            return type == ValueType::anyref || type == ValueType::anyfunc || type == ValueType::nullref;
        }
            ...
            if (subtype == supertype) {
                ...
                switch (supertype) {
                    ...
                    case ValueType::anyfunc:
                        return subtype == ValueType::nullref;
                    default:
                        return false;
                }
            }
        ...
        inline ValueType meet(ValueType a, ValueType b) {
            if (a == b) {
                ...
                if (a == ValueType::nullref || b == ValueType::nullref) {
        ...
        inline std::string asString(I32 value) {
            return std::to_string(value);
        }

        inline std::string asString(I64 value) {
            return std::to_string(value);
        }
        ...
        inline std::string asString(const V128 &v128) {
            ...
            char buffer[48];
            snprintf(buffer, sizeof(buffer), "i32 0x%.8x 0x%.8x 0x%.8x 0x%.8x", v128.u32[0], v128.u32[1], v128.u32[2], v128.u32[3]);
        ...
        struct TypeTuple {
            ...
            const ValueType *end() const {
                return impl->elems + impl->numElems;
            }
        ...
        inline bool isSubtype(TypeTuple subtype, TypeTuple supertype) {
            if (subtype.size() != supertype.size()) {
                ...
                for (Uptr elementIndex = 0; elementIndex < subtype.size(); ++elementIndex) {
                    if (!isSubtype(subtype[elementIndex], supertype[elementIndex])) {
                        return false;
                    }
                }
                return true;
        ...
        template<> constexpr ValueType inferValueType<I32>() {
            return ValueType::i32;
        }

        template<> constexpr ValueType inferValueType<U32>() {
            return ValueType::i32;
        }

        template<> constexpr ValueType inferValueType<I64>() {
            return ValueType::i64;
        }

        template<> constexpr ValueType inferValueType<U64>() {
            return ValueType::i64;
        }

        template<> constexpr ValueType inferValueType<F32>() {
            return ValueType::f32;
        }

        template<> constexpr ValueType inferValueType<F64>() {
        ...
        template<> inline TypeTuple inferResultType<void>() {
            return TypeTuple();
        }
        ...
        struct FunctionType {
            ...
            TypeTuple results() const {
                return impl->results;
            }

            TypeTuple params() const {
                return impl->params;
            }
            ...
            Encoding getEncoding() const {
                return Encoding{reinterpret_cast<Uptr>(impl)};
            }
        ...
        struct IndexedBlockType {
            enum Format {
                noParametersOrResult, oneResult, functionType
            };
            Format format;
            union {
        ...
        inline std::string asString(const GlobalType &globalType) {
            if (globalType.isMutable) {
                return std::string("global ") + asString(globalType.valueType);
            } else {
        ...
        enum class ExternKind : U8 {
            // Standard object kinds that may be imported/exported from WebAssembly modules.
                    function = 0, table = 1, memory = 2, global = 3, exceptionType = 4, max = 4, invalid = 0xff,
        };
        ...
        inline std::string asString(const ExternType &objectType) {
            switch (objectType.kind) {
                case ExternKind::function:
                    return "func " + asString(asFunctionType(objectType));
                case ExternKind::table:
                    return "table " + asString(asTableType(objectType));
                case ExternKind::memory:
                    return "memory " + asString(asMemoryType(objectType));
                case ExternKind::global:
                    return asString(asGlobalType(objectType));
        ...
        enum class CallingConvention {
            wasm, intrinsic, intrinsicWithContextSwitch, c
        };
    }
}
