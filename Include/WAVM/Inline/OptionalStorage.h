...
namespace WAVM {
    ...
    template<typename Contents, bool hasTrivialDestructor = std::is_trivially_destructible<Contents>::value> struct OptionalStorage {
        union {
        ...
        template<typename... Args> OptionalStorage(Args &&... args) : contents(std::forward<Args>(args)...) {
        }
