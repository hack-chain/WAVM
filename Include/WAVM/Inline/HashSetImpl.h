...
bool HashSetIterator<Element>::operator!=(const HashSetIterator &other) {
...
template<typename Element>
HashSetIterator<Element>::operator bool() const {
    return bucket < endBucket && bucket->hashAndOccupancy;
}

template<typename Element>
void HashSetIterator<Element>::operator++() {
    do {
        ++bucket;
...
template<typename Element>
const Element &HashSetIterator<Element>::operator*() const {
