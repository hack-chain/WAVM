...
template<HASHMAP_PARAMETERS> HashMap<HASHMAP_ARGUMENTS>::HashMap(Uptr reserveNumPairs) : table(reserveNumPairs) {
}

template<HASHMAP_PARAMETERS> HashMap<HASHMAP_ARGUMENTS>::HashMap(const std::initializer_list<Pair> &initializerList)
...
template<HASHMAP_PARAMETERS> template<typename... ValueArgs> Value &HashMap<HASHMAP_ARGUMENTS>::getOrAdd(const Key &key, ValueArgs &&... valueArgs) {
    const Uptr hashAndOccupancy = KeyHashPolicy::getKeyHash(key) | HashTableBucket<Pair>::isOccupiedMask;
...
template<HASHMAP_PARAMETERS> template<typename... ValueArgs> bool HashMap<HASHMAP_ARGUMENTS>::add(const Key &key, ValueArgs &&... valueArgs) {
    const Uptr hashAndOccupancy = KeyHashPolicy::getKeyHash(key) | HashTableBucket<Pair>::isOccupiedMask;
    HashTableBucket<Pair> &bucket = table.getBucketForAdd(hashAndOccupancy, key);
    if (!bucket.hashAndOccupancy) {
        bucket.hashAndOccupancy = hashAndOccupancy;
...
template<HASHMAP_PARAMETERS> template<typename... ValueArgs> void HashMap<HASHMAP_ARGUMENTS>::addOrFail(const Key &key, ValueArgs &&... valueArgs) {
    const Uptr hashAndOccupancy = KeyHashPolicy::getKeyHash(key) | HashTableBucket<Pair>::isOccupiedMask;
    HashTableBucket<Pair> &bucket = table.getBucketForAdd(hashAndOccupancy, key);
...
template<HASHMAP_PARAMETERS> template<typename... ValueArgs> Value &HashMap<HASHMAP_ARGUMENTS>::set(const Key &key, ValueArgs &&... valueArgs) {
    const Uptr hashAndOccupancy = KeyHashPolicy::getKeyHash(key) | HashTableBucket<Pair>::isOccupiedMask;
    HashTableBucket<Pair> &bucket = table.getBucketForAdd(hashAndOccupancy, key);
    if (!bucket.hashAndOccupancy) {
        bucket.hashAndOccupancy = hashAndOccupancy;
...
template<HASHMAP_PARAMETERS> bool HashMap<HASHMAP_ARGUMENTS>::contains(const Key &key) const {
    ...
    wavmAssert(!bucket || bucket->hashAndOccupancy == (hash | HashTableBucket<Pair>::isOccupiedMask));
    return bucket != nullptr;
}

template<HASHMAP_PARAMETERS> const Value &HashMap<HASHMAP_ARGUMENTS>::operator[](const Key &key) const {
    const Uptr hash = KeyHashPolicy::getKeyHash(key);
    const HashTableBucket<Pair> *bucket = table.getBucketForRead(hash, key);
    wavmAssert(bucket);
...
template<HASHMAP_PARAMETERS> const HashMapPair <Key, Value> *HashMap<HASHMAP_ARGUMENTS>::getPair(const Key &key) const {
    const Uptr hash = KeyHashPolicy::getKeyHash(key);
    const HashTableBucket<Pair> *bucket = table.getBucketForRead(hash, key);
    if (bucket) {
        wavmAssert(bucket->hashAndOccupancy == (hash | HashTableBucket<Pair>::isOccupiedMask));
...
template<HASHMAP_PARAMETERS> void HashMap<HASHMAP_ARGUMENTS>::clear() {
    table.clear();
}
...
template<HASHMAP_PARAMETERS> Uptr HashMap<HASHMAP_ARGUMENTS>::size() const {
    return table.size();
}

template<HASHMAP_PARAMETERS> void HashMap<HASHMAP_ARGUMENTS>::analyzeSpaceUsage(Uptr &outTotalMemoryBytes, Uptr &outMaxProbeCount, F32 &outOccupancy, F32 &outAverageProbeCount) const {
    return table.analyzeSpaceUsage(outTotalMemoryBytes, outMaxProbeCount, outOccupancy, outAverageProbeCount);
}

template<typename Key, typename Value> template<typename... ValueArgs> HashMapPair<Key, Value>::HashMapPair(const Key &inKey, ValueArgs &&... valueArgs)
        : key(inKey), value(std::forward<ValueArgs>(valueArgs)...) {
}

template<typename Key, typename Value> template<typename... ValueArgs> HashMapPair<Key, Value>::HashMapPair(Key &&inKey, ValueArgs &&... valueArgs)
        : key(std::move(inKey)), value(std::forward<ValueArgs>(valueArgs)...) {
}

template<typename Key, typename Value> bool HashMapIterator<Key, Value>::operator!=(const HashMapIterator &other) {
...
template<typename Key, typename Value> const HashMapPair <Key, Value> &HashMapIterator<Key, Value>::operator*() const {
    wavmAssert(bucket->hashAndOccupancy);
    return bucket->storage.contents;
}
...
template<typename Key, typename Value> HashMapIterator<Key, Value>::HashMapIterator(const HashTableBucket <HashMapPair<Key, Value>> *inBucket, const HashTableBucket <HashMapPair<Key, Value>> *inEndBucket)
