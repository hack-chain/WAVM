...
template<HASHMAP_PARAMETERS>
bool HashMap<HASHMAP_ARGUMENTS>::remove(const Key &key) {
    return table.remove(KeyHashPolicy::getKeyHash(key), key);
}

template<HASHMAP_PARAMETERS>
void HashMap<HASHMAP_ARGUMENTS>::removeOrFail(const Key &key) {
    const bool removed = table.remove(KeyHashPolicy::getKeyHash(key), key);
    wavmAssert(removed);
}

template<HASHMAP_PARAMETERS>
bool HashMap<HASHMAP_ARGUMENTS>::contains(const Key &key) const {
...
template<HASHMAP_PARAMETERS>
const Value *HashMap<HASHMAP_ARGUMENTS>::get(const Key &key) const {
    const Uptr hash = KeyHashPolicy::getKeyHash(key);
    const HashTableBucket<Pair> *bucket = table.getBucketForRead(hash, key);
    if (bucket) {
        wavmAssert(bucket->hashAndOccupancy == (hash | HashTableBucket<Pair>::isOccupiedMask));
...
template<HASHMAP_PARAMETERS>
void HashMap<HASHMAP_ARGUMENTS>::clear() { table.clear(); }

template<HASHMAP_PARAMETERS>
HashMapIterator <Key, Value> HashMap<HASHMAP_ARGUMENTS>::begin() const {
...
template<HASHMAP_PARAMETERS>
HashMapIterator <Key, Value> HashMap<HASHMAP_ARGUMENTS>::end() const {
...
template<typename Key, typename Value>
HashMapIterator<Key, Value>::operator bool() const {
    return bucket < endBucket && bucket->hashAndOccupancy;
}

template<typename Key, typename Value>
void HashMapIterator<Key, Value>::operator++() {
    do {
        ++bucket;
